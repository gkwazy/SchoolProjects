...
class EncFS(Operations):
    ...
    def __init__(self, root):
        ...
        self.netFD = 3
        #TODO, FINISH ME!!
        self.descriptorcounter = 0
        self.startingDictionary = {}
        self.userPassword = getpass(prompt='Password: ')
        # self.userPassword = bytes(self.userPassword);
        print('Password entered:', self.userPassword)
    ...
    def getattr(self, path, fh=None):
        ...
        wantedfile = open(full_path, "rb")
        fileobject = wantedfile.read()
        decryptFile = self.myDecrypt(fileobject)
        wantedfile.close()
        print("size here", sys.getsizeof(decryptFile))
        print("size here", st.st_size)
        print("FIXME FIXME: update so that the size reported here is the length of the decrypted data, not the physical file size!!")
        newdict = dict((key, getattr(st, key)) for key in ('st_atime', 'st_ctime',
               'st_gid', 'st_mode', 'st_mtime', 'st_nlink', 'st_size', 'st_uid'))
        newdict['st_size'] = sys.getsizeof(decryptFile)
        return newdict
...
'''
    ...
    def open(self, path, flags):
        ...
        if not os.path.exists(path):
            print("bad path")
            return -1
        elif self.startingDictionary.get(path) == "None":
            print("already open")
            return -1
        else:
            wantedfile = open(path, "rb")
            fileobject = wantedfile.read()
            decryptFile = self.myDecrypt(fileobject)
            self.startingDictionary[oldPath] = decryptFile
            wantedfile.close()
            print("opened: ", decryptFile)
            self.descriptorcounter += 1
        return self.descriptorcounter

    def myDecrypt(self,fileobject):
        filearray = bytearray(fileobject)
        filesalt = bytes(filearray[:16])
        filearraynosalt = bytes(filearray[16:])
        password = bytes(self.userPassword, encoding='UTF-8')
        salt = os.urandom(16)
        kdf = PBKDF2HMAC(
        ...
        f = Fernet(key)
        decryptFile = f.decrypt(filearraynosalt)
        return decryptFile

    def myEncrypt(self, fileobject):
        filearray = bytearray(fileobject)
        filearraynosalt = bytes(filearray[16:])
        password = bytes(self.userPassword, encoding='UTF-8')
        salt = os.urandom(16)
        ...
        key = base64.urlsafe_b64encode(kdf.derive(password))
        f = Fernet(key)
        encryptFile = f.encrypt(filearraynosalt)
        return encryptFile
    ...
    def create(self, path, mode, fi=None):
        ...
        self.startingDictionary[path] = 0
        file.close()
        self.descriptorcounter += 1
        return self.descriptorcounter
    ...
    def read(self, path, length, offset, fh):
        ...
        """
        print("Read is open")
        return self.startingDictionary[path][offset: offset + length]
    ...
    def write(self, path, buf, offset, fh):
        ...
        print("path:", path)
        print("buf",buf)
        # wantedfile = open(path, "a")
        message = self.myEncrypt(self.startingDictionary[path:offset])
        # wantedfile.write(encryptMessage)
    ...
    def truncate(self, path, length, fh=None):
        ...
        """
        print("truncate is open")
        print("path:", path)
        print("length", length)
        oldSize = sys.getsizeof(self.startingDictionary[path])
        newSize = self.startingDictionary[path][:oldSize + length]
        print("old size", sys.getsizeof(self.startingDictionary[path]))
        print("new size", sys.getsizeof(newSize))
        return newSize
   ...
   '''                                            
    ...
    def release(self, path, fh):
        ...
        encryptmessage  = self.myEncrypt(self.startingDictionary[path])
        wantedfile = open(path, "a")
        print("realse path:", path)
        wantedfile.write(encryptmessage)
        wantedfile.close()
        del self.startingDictionary[path]
        return
