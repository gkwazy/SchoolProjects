...
if(FindMin(v1) != -1)
    ErrorExit("FindMin did not find min value: Test 3");
    
if(FindMin(v1) == 2 )
    ErrorExit("FindMin found max not min: Test 4");

if(FindMin(v2) != 1)
        ErrorExit("FindMin did not find min value: Test 5");
        
if(FindMin(v2) == 3 )
        ErrorExit("FindMin found max not min: Test 6");
    
if(FindMax(v1) != 5)
    ErrorExit("FindMax did not find max value: Test 7");
    
if(FindMax(v1) != 5)
    ErrorExit("FindMax found the min value: Test 8");
if(FindMax(v2) != 3)
    ErrorExit("FindMax: Test 9");

if(Average(v1) != 1)
    ErrorExit ("Average: Test 10");
if(Average(v2) != 2)
    ErrorExit("Average: Test 11");
if (IsSorted(v1) == true)
    ErrorExit("IsSorted: Test 12");
if(IsSorted(v2) == false)
    ErrorExit("IsSorted: Test 13");
if(IsSorted(empty) == false)
    ErrorExit("IsSorted: Test 14");
