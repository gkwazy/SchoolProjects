...
//error checking ">" or "<" commands are in incorrect locations;
          if(i >= last){
              perror("miss formated");
              error = true;
              break;
          }
          ...
          if (tokens[j] == "<"){
              ...
              if (i != ret.size() - 1){
                  perror("> placed in wrong location");
                  error = true;
                  break;
              }
              ret[i].fdStdout = open(tokens[j+1].c_str(), O_WRONLY, O_CREAT, O_APPEND, 777);
          }
 // if not ">" or "<" then error has happened
          else{
              error = true;
              break;
          }
