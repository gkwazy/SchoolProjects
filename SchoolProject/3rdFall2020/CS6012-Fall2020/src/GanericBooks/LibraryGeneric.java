...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		for (int i = 0; i < library.size(); i++) {
			long wantedIsbn = library.get(i).getIsbn();
			if (wantedIsbn == isbn) {
				Type wantedHolder = library.get(i).getHolder();
				return wantedHolder;
			}
		}
		return null;
		...
		for (int i = 0; i < library.size(); i++) {
			...
			if (wantedIsbn == isbn) {
				if (library.get(i).getHolder() == null) {
					library.get(i).checkBookOut(date, holder);
				}
				return true;
			}
		}
	...
	public ArrayList<LibraryBookGeneric<Type>> getOrderedByAuthor() {
		...
		ArrayList<LibraryBookGeneric<Type>> tempArray = new ArrayList<LibraryBookGeneric<Type>>();
		tempArray.addAll(library);
		OrderByAuthor comparator = new OrderByAuthor();
		sort(tempArray, comparator);
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		ArrayList<LibraryBookGeneric<Type>> tempArray = new ArrayList<LibraryBookGeneric<Type>>();
		GregorianCalendar wantedDate = new GregorianCalendar(year, month, day);
		for (int i = 0; i < library.size(); i++) {
			if (library.get(i).getDueDate() != null && library.get(i).getDueDate().compareTo(wantedDate) <= 0) {
				tempArray.add(library.get(i));
			}
		}
